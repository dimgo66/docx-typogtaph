...
- **Шаги выполнения**:
  - [x] Определить типы ошибок LanguageTool, которые будут передаваться ИИ для исправления.
  - [x] Разработать функцию для взаимодействия с API ИИ (OpenRouter, модель qwen/qwen3-235b-a22b:free) в `AutocorrectProcessorModule.py`.
    - [x] Передача текста ошибки и контекста.
    - [x] Получение и обработка предложенного исправления.
    - [x] Использование переменных окружения `OPENROUTER_API_KEY`, `YOUR_SITE_URL`, `YOUR_APP_NAME`.
  - [ ] Модифицировать `tools/check_text_languagetool.py`: (Этот шаг, возможно, уже не актуален, т.к. логика перенесена в `AutocorrectProcessorModule`)
