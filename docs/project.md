...
### Контроль языка для LanguageTool
В пайплайне функция ensure_html_lang_ru вызывается непосредственно в модуле LanguageToolProcessorModule перед запуском проверки. Это гарантирует, что любой HTML-файл, поступающий на этот этап, всегда содержит lang="ru" в теге <html>, и исключает ошибки LanguageTool, связанные с определением языка после любых промежуточных преобразований.

### Архитектурные изменения (2024-07-05)
- Удалён fallback-этап: python-docx больше не используется для аварийного извлечения текста из DOCX. Все конвертации DOCX → HTML выполняются только через Pandoc. Это упрощает пайплайн и устраняет дублирование логики.
...
### Обработка HTML для LanguageTool
Перед запуском проверки текста с помощью LanguageTool пайплайн автоматически проверяет и, при необходимости, устанавливает lang="ru" в теге <html> каждого HTML-файла. Это предотвращает ошибки, связанные с определением языка, и обеспечивает корректную работу проверки.
...
### Исследование Render.com (2025-06-04)
В связи с постоянными проблемами нехватки оперативной памяти для LanguageTool на бесплатном тарифе Railway (лимит 0.3 ГБ RAM), принято решение исследовать развертывание на платформе Render.com. Бесплатный тариф Render предоставляет 0.5 ГБ RAM, что дает больше шансов на стабильную работу LanguageTool.

Настройка памяти для Java (LanguageTool) в `languagetool_processor_module.py` адаптирована под этот лимит и установлена в `-Xmx256m`.
Дальнейшие шаги включают настройку сервиса на Render, перенос переменных окружения и тестирование пайплайна.
